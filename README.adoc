...
for any changes in the project, including application source code or configuration. If a Dockerfile is present 
development mode with will be used to build the image (container?), otherwise the default Open Liberty image, 
`openliberty/open-liberty:kernel-java8-openj9-ubi`, will be used.


Build and run the container by running the `devc` goal in the `start` directory
...
After you make the file changes, Open Liberty automatically restarts(?) the container. To see these changes reflected in the container, 
...
The FROM instruction initializes a new build stage and indicates the parent image from which your image
is built. If you don’t need a parent image, then use FROM scratch, which makes your image a base image.
...
Run the following command to inspect the descripton `field` of the `LABEL` that you specified in the Dockerfile 
...
for the `docker run` command. This can be done using the `-DdockerRunOpts` option with the `devc` command.
Any options that you would normally add to the `docker run` command, can be added to this options. Below
is an example of an environment variable being passed in.

[role="no_copy"]
----
mvn liberty:devc -DdockerRunOpts="--env VAR=123"
----

If the dockerfile isn't located in the directory that the `devc` goal is being run from, you can also 
the `-Ddockerfile` goal to specify the location. 

Additionally, both of these options can be added to the `pom.xml` file as part under the `<configuration>` 
tab of the Liberty Maven Plugin as such:
...
When you are finished, press `CTRL+C` in the session that the server is running in. This will both stop and remove the container for you. 
