...
we must temporarily switch to the `root` user to create the symlink. This is done by using 
the [hotspot=4 hotspot=7 file=0]`USER` instruction. 

The [hotspot=10 file=0]`ENTRYPOINT` and [hotspot=9]`CMD` instructions define a default command that executes when the image runs as
...
We use the `-u` flag to override the default user within the container to match the user 
