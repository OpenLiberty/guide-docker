...
Your [hotspot]`pom.xml` file is already configured to add your REST application to the `defaultServer`. But you can tweak this configuration or add your own for another server by updating the [hotspot=execution]`<execution/>` element.

The [hotspot=install-apps]`install-apps` goal copies the application into the specified directory of the specified server.
In this case, the goal copies the `rest.war` file into the [hotspot=apps]`apps` directory of the `defaultServer` server.
...
The [hotspot=entrypoint file=0]`ENTRYPOINT` and [hotspot=default-start]`CMD` instructions define a default command that executes when the image runs as
a container. These two instructions function the same way, except that the [hotspot=default-start file=0]`CMD` instruction is overridden
with any arguments that are passed at the end of the `docker run` command. In contrast, the
[hotspot=entrypoint file=0]`ENTRYPOINT` instruction requires the `--entrypoint` flag to be overridden. In this case, you use
the [hotspot=entrypoint file=0]`ENTRYPOINT` instruction to start an Open Liberty server and the [hotspot=default-start file=0]`CMD` instruction to indicate
which server to start. Because the [hotspot=default-start file=0]`CMD` instruction is easily overridden, starting any server is convenient.
...
A [hotspot file=0]`.dockerignore` file is available to you in the `start` directory. This file includes the [hotspot=src file=0]`src`
...
7f24bdb73d53: Pull complete 
264aaa453271: Pull complete 
b095666e9645: Pull complete 
6d080a23bd35: Pull complete 
7bdfdc073f3b: Pull complete 
7cf1f897277a: Pull complete 
38693ac81335: Pull complete 
1207f8b498f1: Pull complete 
531978dfef2c: Pull complete 
b67203a348c6: Pull complete 
d0386355392b: Pull complete 
Digest: sha256:676f0137e439e42b8f3250060a1ec2c4b9ae0ca851a4cb3cb2683256add838ec
...
Next, to build your image, make sure that your Docker daemon is running and execute the Docker `build`
command from the active command-line session. If you execute your build from the same directory as your Dockerfile,
you can use the period character (`.`) notation to specify the location for the build context. 
Otherwise, use the `-f` flag to point to your Dockerfile:
...
[role="no_copy"]
----
Sending build context to Docker daemon  316.5MB
Step 1/6 : FROM openliberty/open-liberty:kernel-java8-openj9-ubi
 ---> c3cd510a72a2
Step 2/6 : USER root
 ---> Running in a63a1bed8026
Removing intermediate container a63a1bed8026
 ---> ace3b840f2fd
Step 3/6 : RUN ln -s /opt/ol/wlp/usr/servers /servers
 ---> Running in 504b98ddff9a
Removing intermediate container 504b98ddff9a
 ---> 1628ec9873b1
Step 4/6 : USER 1001
 ---> Running in 1d8601e3896e
Removing intermediate container 1d8601e3896e
 ---> a75cddf7cfc6
Step 5/6 : ENTRYPOINT ["/opt/ol/wlp/bin/server", "run"]
 ---> Running in 231f5b20fb0c
Removing intermediate container 231f5b20fb0c
 ---> 87501190599c
Step 6/6 : CMD ["defaultServer"]
 ---> Running in 9377b2104726
Removing intermediate container 9377b2104726
 ---> d8fd3f885f8f
Successfully built d8fd3f885f8f
...
--
Make sure you substitute the absolute path to this guide.
...
[role='command']
```
docker run -d --name rest-app -p 9080:9080 -p 9443:9443 -v $(pwd)/target/liberty/wlp/usr/servers/defaultServer:/servers/defaultServer ol-runtime
```
--
...
[role='command']
```
docker run -d --name rest-app -p 9080:9080 -p 9443:9443 -v $(pwd)/target/liberty/wlp/usr/servers/defaultServer:/servers/defaultServer -u `id -u` ol-runtime
```
--
...
Learn more about running containers on the https://docs.docker.com/engine/reference/run[official Docker website^]
...
Before you access your application from the browser, run the `docker ps` command from the active
command-line session to make sure that your container is running and didn't crash:
...
To view a full list of all available containers, run the `docker ps -a` command in the command-line session.
...
To see these changes reflected in the container, run the following command in the command-line session to
...
To stop your container, run the following command in the command-line session.
[role='command']
```
