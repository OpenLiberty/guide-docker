...
First, run the Maven package goal to build the application `.war` file. 

To try out the application that is going to be in inside your Docker container, navigate to the 
`start` directory are the the following Maven goal. This will build you application and run it inside 
Open Liberty:
...
When you are done checking out the application, package it so that it is ready to be run from a Docker 
container. Run the Maven package goal to build the application .war file in the `target` directory.
