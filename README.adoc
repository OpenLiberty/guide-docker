...
This guide will introduce development mode with a container. With a single command, this mode automatically builds 
a Docker image and runs it as a container, all while listening for changes in the source code and configuration.
...
To try out the application that is going to be in inside your Docker container, navigate to the 
`start` directory are the the following Maven goal. This will build your application and run it inside 
Open Liberty:
...
Point your browser to the localhost:9080/LibertyProject/System/properties[localhost:9080/LibertyProject/System/properties^] URL where 
you can see a JSON file that contains the system properties of the JVM in your container.
...
Run the following command to inspect the [hotspot=description file=0]`descripton` field
of the [hotspot=label file=0]`LABEL` that you specified in the Dockerfile 
...
to the `docker run` command. You can do this by adding the `<dockerRunOpts>` tag to the `pom.xml` under 
