...
[role="code_command hotspot=163-174", subs="quotes"]
----
But you can tweak this configuration or add your own for another server here.
----
...
Change the endpoint of your application from `/properties` to `/properties-new` by replacing the `PropertiesResource.java` class and changing the `@Path`
annotation to `"properties-new"`.*
[role="code_command hotspot=30-31", subs="quotes"]
----
The endpoint of your application is changed from `/properties` to `/properties-new` because the `@Path` annotation is set to `"propertiess-new"`.
----
