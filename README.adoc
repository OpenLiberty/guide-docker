...
The [hotspot=1-2]`FROM` instruction initializes a new build stage and indicates the parent image from which your
...
The [hotspot=4-5]`RUN` instruction executes various shell commands in a new layer on top of the current image.
...
The [hotspot=9-10]`CMD` and [hotspot=7-8]`ENTRYPOINT` instructions define a default command that executes when the image runs as
...
Change the endpoint of your application from [hotspot=17-18]`/properties` to `/properties-new` by replacing the `PropertiesResource.java` class and changing the `@Path`
