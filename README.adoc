...
mvn clean install
```
//[role="code_command hotspot=1-228", subs="quotes"]
//----
//Take a look at your `pom.xml` file.
//----
...
----
But you can tweak this configuration or add your own for another server [hotspot=163-174]`here`.
...
----
Edit the `Properties Resource` class file.
...
Change the endpoint of your application from [hotspot=18]`/properties` to `/properties-new` by replacing the `PropertiesResource.java` class and changing the `@Path`
