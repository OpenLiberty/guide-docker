...
Your [hotspot=1-228]`pom.xml` file is already configured to add your REST application to the `defaultServer`. But you can tweak this configuration or add your own for another server by updating the [hotspot=168-179]`<execution/>` element.
...
[role="code_command hotspot=1-9",subs="quotes"]
----
Create the `Dockerfile`.
`start/Dockerfile`
----

The [hotspot=2]`FROM` instruction initializes a new build stage and indicates the parent image from which your
...
The [hotspot=5]`RUN` instruction executes various shell commands in a new layer on top of the current image.
...
The [hotspot=8]`ENTRYPOINT` and [hotspot=9]`CMD` instructions define a default command that executes when the image runs as
a container. These two instructions function the same way, except that the [hotspot=9]`CMD` instruction is overridden
with any arguments that are passed at the end of the `docker run` command. In contrast, the
[hotspot=8]`ENTRYPOINT` instruction requires the `--entrypoint` flag to be overridden. In this case, you use
the [hotspot=8]`ENTRYPOINT` instruction to start an Open Liberty server and the [hotspot=9]`CMD` instruction to indicate
which server to start. Because the [hotspot=9]`CMD` instruction is easily overridden, starting any server is convenient.
...
Now that your image is built, execute the Docker `run` command with the absolute path to this guide, from the command line to run it:
...
[role="code_command hotspot=19-36", subs="quotes"]
----
Edit the `PropertiesResource` class.
`start/src/main/java/io/openliberty/guides/rest/PropertiesResource.java`
----
...
You containerized a simple REST application in Open Liberty. You can now continuously deliver changes to the application,
