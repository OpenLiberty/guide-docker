...
Your [hotspot=1-228]`pom.xml` file is already configured to add your REST application to the `defaultServer`. But you can tweak this configuration or add your own for another server by updating the [hotspot=172-179]`<execution/>` element.
...
Change the endpoint of your application from [hotspot=18]`properties` to `properties-new` by changing the `@Path`
